...
class CasoService:
    def criar_caso(self, nome, raca, idade, descricao, foto=None):
        """
        foto: string base64 ou None
        """
    ...
    def get_casos(self):
        ...
        for caso in casos:
            result.append({
                ...
                "foto": caso.foto  # string base64 ou None
            })
        return result
    ...
    def update_caso(self, id, nome, raca, idade, descricao, foto=None):
        caso = Caso.query.filter_by(id=id).first()
        if not caso:
            return None
